...
- Cloud model: `--dart-define=OCR_CLOUD_MODEL=...`, default `google/gemma-4-26b-a4b-it` (vision-capable, cheap).
...
curl -s http://localhost:8787/v1/chat/completions \
  ...
  -d '{"model":"google/gemma-4-26b-a4b-it","messages":[{"role":"user","content":"say ok"}]}' \
  | head -c 300
```
...
curl -s http://localhost:8787/v1/chat/completions \
  -H 'Content-Type: application/json' \
  -d '{"model":"google/gemma-4-26b-a4b-it","messages":[{"role":"user","content":"say ok"}]}' | head -c 300
```
...
void main() {
  test('CloudOcrEngine posts OpenAI vision payload and returns content',
    ...
    expect(capturedBody['model'], 'google/gemma-4-26b-a4b-it');
...
- Produces: a rewritten `receiptOcrEngineProvider` that returns `CloudOcrEngine` when `resolveBackend(...) == Backend.online`, else `GemmaOcrEngine`. Cloud config from dart-defines `OCR_PROXY_URL` (default `http://localhost:8787`) and `OCR_CLOUD_MODEL` (default `google/gemma-4-26b-a4b-it`).
...
cd apps/mobile && flutter run -d <ios-simulator-id> \
  ...
  --dart-define=OCR_CLOUD_MODEL=google/gemma-4-26b-a4b-it
```
